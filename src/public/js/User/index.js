...
export default class User {
  ...
  initialize = async () => {
    ...
    $('#changeUserAvatar').on('change', (e) => {
      ...
      $(`#participant-${store.getCurrentUser().socketId}`).css(
        'background-color',
        avatar
      );
  ...
  getRandomColor = () => {
    ...
    return `#${randomColorString}${'0'.repeat(
      6 - randomColorString.length
    )}`.substring(0, 7);
  };
  ...
  renderParticipantAvatar = () => {
    ...
    const $avatar = $(`#participant-${this.socketId}`).length
      ? $(`#participant-${this.socketId}`)
